    ...
    switch ( $post_type )
    {
        ...
        case 'post':
            ...
            foreach ( $columns as $key => $value )
            {
                ...
                if ( $key == 'title' )
                {
                    #$new_columns[ 'test' ] = 'Testdaten';
                    $new_columns[ 'cover' ] = 'Poster';
        ...
        case 'ultraschall_team':
            ...
            foreach ( $columns as $key => $value )
            {
                ...
                {
                    #$new_columns[ 'test' ] = 'Testdaten';
                    $new_columns[ 'photo' ] = 'Foto';
                    $new_columns[ 'description' ] = 'Beschreibung';
                    $new_columns[ 'social' ] = 'Netzwerke';
                    #$new_columns[ 'release_date_column' ] = 'Release Date';
                    #$new_columns[ 'coming_soon_column' ] = 'Coming Soon';
                    #$new_columns[ 'film_rating_column' ] = 'Film Rating';
                }
...
function manage_wp_posts_be_qe_manage_sortable_columns( $sortable_columns ) {
    ...
    $sortable_columns[ 'cover' ] = 'cover_data';

    //Team Person
    $sortable_columns[ 'photo' ] = 'user_photo_data';
    $sortable_columns[ 'description' ] = 'user_description_data';
    $sortable_columns[ 'social' ] = 'user_social_data';
...
function manage_wp_posts_be_qe_manage_posts_custom_column( $column_name, $post_id ) {
    ...
    switch ( $column_name )
    {
        ...
        /*case 'test':
            ...
            echo '<div id="test_date-' . $post_id . '">' . get_post_meta( $post_id, 'test_date', true ) . '</div>';
            break;
        */

        case 'cover':

            echo '<div id="cover-' . $post_id . '">
					<img style="border: 1px solid #eee; width: 60px; height: 60px;" src="' .
        ...
        case 'photo':
            ...
            echo '<div id="photo-' . $post_id . '">
					<a href="post.php?post=' . $post_id . '&action=edit">
					<img style="border: 1px solid #eee; width: 60px; height: 60px;" src="' . get_field( 'photo' ) . '" />
                    </a>
				</div>';
            break;
        ...
        case 'description':
            ...
            $desc_content = get_field( 'description' );
            $desc_count = strlen($desc_content);
            $desc_count_negativ = strlen($desc_content)-50;
            $new_desc = preg_replace("/[^ ]*$/", '', substr($desc_content, 0, $desc_count - $desc_count_negativ));

            echo '<div id="description-' . $post_id . '">
					<p>' . $new_desc . '... </p>
				</div>';
        ...
        case 'social':
            ...
            echo '<div id="social-' . $post_id . '">';
            $rows = get_field( 'networks' );
            if ( $rows )
            {
                foreach ( $rows as $row )
                {
                    echo '[<a href="' . $row[ 'url' ] . '">' . $row[ 'network' ] . '</a>] ';
                }
            }
            echo '</div>';
    ...
    if ( $query->is_main_query() && ( $orderby = $query->get( 'orderby' ) ) )
    {
        ...
        switch ( $orderby )
        {
            ...
            case 'cover_data':
                ...
                // set our query's meta_key, which is used for custom fields
                $query->set( 'meta_key', 'cover' );
                echo get_field( 'cover' );
            ...
            case 'user_photo_data':

                // set our query's meta_key, which is used for custom fields
                $query->set( 'meta_key', 'photo' );
            ...
            case 'user_social_data':

                // set our query's meta_key, which is used for custom fields
                $query->set( 'meta_key', 'social' );
    ...
    if ( $query->is_main_query() && ( $orderby = $query->get( 'orderby' ) ) )
    {
        ...
        switch ( $orderby )
        {
            ...
            case 'test_date':
                 ...
                 */
                $pieces[ 'join' ] .= " LEFT JOIN $wpdb->postmeta wp_rd ON wp_rd.post_id = {$wpdb->posts}.ID AND wp_rd.meta_key = 'test_date'";
            ...
            case 'cover_data':
                 ...
                 * our release date in the query.
                 */
                $pieces[ 'join' ] .= " LEFT JOIN $wpdb->postmeta wp_rd ON wp_rd.post_id = {$wpdb->posts}.ID AND wp_rd.meta_key = 'cover'";
            ...
            case 'user_photo_data':
                 ...
                 * our release date in the query.
                 */
                $pieces[ 'join' ] .= " LEFT JOIN $wpdb->postmeta wp_rd ON wp_rd.post_id = {$wpdb->posts}.ID AND wp_rd.meta_key = 'photo'";
            ...
            case 'user_social_data':
                 ...
                 * our release date in the query.
                 */
                $pieces[ 'join' ] .= " LEFT JOIN $wpdb->postmeta wp_rd ON wp_rd.post_id = {$wpdb->posts}.ID AND wp_rd.meta_key = 'social'";
                ...
                case 'test_date':
                    ...
                    <fieldset class="inline-edit-col-left">
                        <div class="inline-edit-col">
                            <label>
                                <span class="title">Testdate</span>
								<span class="input-text-wrap">
									<input type="text" value="" name="test_date">
								</span>
                            </label>
                        </div>
                    </fieldset><?php
                    ...
                    <fieldset class="inline-edit-col-left">
                        <div class="inline-edit-col">
                            <label>
								...
								<span class="input-text-wrap">
									<input type="text" value="" name="cover">
								</span>
                            </label>
                        </div>
                    </fieldset><?php
                    ...
                    <fieldset class="inline-edit-col-left">
                        <div class="inline-edit-col">
                            <label>
								...
								<span class="input-text-wrap">
									<input type="text" value="" name="social">
								</span>
                            </label>
                        </div>
                    </fieldset><?php
...
function manage_wp_posts_be_qe_enqueue_admin_scripts() {
    ...
    wp_enqueue_script( 'manage-wp-posts-using-bulk-quick-edit',
            trailingslashit( get_template_directory_uri() ) . 'assets/js/bulk_quick_edit.js',
...
function manage_wp_posts_be_qe_save_post( $post_id, $post ) {
    ...
    switch ( $post->post_type )
    {
        ...
        case 'post':
             ...
             */
            $custom_fields = array( 'test_date', 'cover_data' );
